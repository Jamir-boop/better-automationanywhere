...
Navigation commands are generated from sidebar views available to the current user. The script reads internal sidebar links from `Pathfinder.primaryItems` and creates commands from each view label, title, aria-label, and name.

If a view is not present in the sidebar, no command is generated for it.

Examples when those sidebar views are available:

- `home`, `dashboard`, `overview` — Go to **Home**.
- `historical`, `history`, `activity historical` — Go to **Historical**.
- `inprogress`, `progress`, `in progress` — Go to **In progress**.
- `audit`, `audit log` — Go to **Audit log**.
- `users`, `admin users`, `manage users` — Go to **Users**.
- `roles`, `admin roles`, `manage roles` — Go to **Roles**.
- `devices`, `admin devices`, `manage devices` — Go to **Devices**.
- `pack`, `packages` — Go to **Packages**.
- `oauth`, `oauth connections` — Go to **OAuth connections**.
- `p`, `private`, `private bots` — Go to **Private** only if that sidebar view exists.
- `pub`, `public`, `public bots` — Go to **Public** only if that sidebar view exists.
